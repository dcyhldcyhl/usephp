...
namespace system\lib\db{
	 ...
	 *
	 */
	class dbMysql extends core\base 
	{
		...
		private static function get_master_link()
		{
			if(!self::$db_master_link)
			{
				...
				self::$db_master_link = mysql_connect($params['host'], $params['username'], $params['password']);
				if(!self::$db_master_link)
				{
				...
				//设置数据库
				mysql_select_db($params['dbname'],self::$db_master_link);
				mysql_query("SET NAMES {$params['charset']}",self::$db_master_link);
			}
		...
		private static function get_slave_link()
		{
			if(!self::$db_slave_link)
			{
				...
				self::$db_slave_link = mysql_connect($params['host'], $params['username'], $params['password']);
				...
				//设置数据库
				mysql_select_db($params['dbname'],self::$db_slave_link);
				mysql_query("SET NAMES {$params['charset']}",self::$db_slave_link);				
			}
		...
		{			
			...
			{
				$result = mysql_query($sql,self::get_slave_link());
			}
			else 
			{
	...
	/**
		...
		{		
	        $lastId = mysql_insert_id(self::get_master_link());
	        if (($lastId = mysql_insert_id($conn)) > 0)
	        {
	            return $lastId;
	        }
	        return self::getRow("SELECT LAST_INSERT_ID()",true);
		}
